...
| [bun](https://bun.sh) ≥1.1 (only for local builds) | `bun --version` | `mise use -g bun@latest` or `sudo pacman -S bun` |

Marketplace installs do **not** require `bun` if a release commits `bin/yfocus` (`git add -f bin/yfocus` — `bun build --compile` is a static ELF with no runtime). Source checkouts need `bun`.
...
bun test              # queue + model parity
./build.sh              # dev: bun build --compile --target bun-linux-x64 → bin/yfocus (~80 MB)
# or: make bundle       # x86_64 + aarch64 → bin/yfocus-* + shim + .sha256 + .srcid (like obsidian-daily-qs)
#     make verify-bundle
```

`build.sh` fails fast if `bun` is missing. The ELF keeps its symbol table (no strip) so `nm` can inspect.
...
repairing state. The plugin resolves `bin/yfocus` (bundled) → `yfocus` on PATH → `YFOCUS_BIN` override.
...
cd ~/coding/personal/plugins/yfocus
./build.sh            # dev: bun build → bin/yfocus
# or: make bundle     # reproducible x86_64 + aarch64 → bin/yfocus-* + shim (or use a release that already has bin/*)
ln -sfn "$PWD" "$HOME/.config/omarchy/plugins/You-ne5.yfocus"
...
- `bin/yfocus` **should** be committed on releases (`git add -f bin/yfocus` — ~80 MB). No per-machine rebuild needed if the clone already has it.
...
3. **Binary strategy**: `make bundle` then `make verify-bundle` (bun x86_64 + aarch64, `sha256` + `.srcid` attested, keep symbols for `nm` — like obsidian-daily-qs). Commit `bin/yfocus*` in the same commit as `ts/` changes. CI runs `verify-bundle` on every push. Or document `bun` as post-install requirement and keep `bin/` gitignored (less reproducible).
...
```bash
omarchy plugin add https://github.com/You-ne5/yfocus.git --enable --yes
```
...
Bump `version` in `manifest.json` (and `CHANGELOG.md`) for every published change; if you changed `ts/` run `./build.sh` and `git add -f bin/yfocus` in the same commit; the shell and `omarchy plugin update` both surface it. Keep `schemaVersion: 1` unless the plugin manifest contract itself changes upstream.
...
| `yfocus: command not found` | `plugin add` never puts `yfocus` on `PATH` (same as `obsidian-daily-qs`). Run `make install` (`sudo make install` → `/usr/local/bin`) or `make install PREFIX=$HOME/.local` (`~/.local/bin`, no sudo) or `ln -sf ~/.config/omarchy/plugins/You-ne5.yfocus/bin/yfocus ~/.local/bin/yfocus` or set `YFOCUS_BIN`. |
| Mutations don't persist | `YFOCUS_BIN` unset and `bin/yfocus` not built → run `./build.sh` (needs `bun`) or `make bundle` (commits `bin/yfocus-*`) or set `YFOCUS_BIN`. |
